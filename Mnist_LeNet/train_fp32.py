...
if __name__ == "__main__":
    ...
    # conv1
    layer1_weights = tf.compat.v1.Variable(tf.compat.v1.truncated_normal([5, 5, 1, 128], stddev=0.05))
    layer1_bias = tf.compat.v1.Variable(tf.compat.v1.constant(0.1, shape=[128]))
    ...
    # conv2
    layer2_weights = tf.compat.v1.Variable(tf.compat.v1.truncated_normal([5, 5, 128, 64], stddev=0.05))
    ...
    layer2_flat = tf.compat.v1.reshape(layer2_pool, [-1, 7 * 7 * 64])  # 展开成一维，进行全连接层的计算

    # # FC
    # layer3_weights = tf.compat.v1.Variable(tf.compat.v1.truncated_normal([7 * 7 * 64, 128], stddev=0.05))
    # layer3_bias = tf.compat.v1.Variable(tf.compat.v1.constant(0.1, shape=[128]))
    ...
    # FC2
    layer4_weights = tf.compat.v1.Variable(tf.compat.v1.truncated_normal([7 * 7 * 64, 10], stddev=0.05))
    layer4_bias = tf.compat.v1.Variable(tf.compat.v1.constant(0.1, shape=[10]))
    layer4_optput = tf.compat.v1.nn.bias_add(tf.compat.v1.matmul(layer2_flat, layer4_weights), layer4_bias)
